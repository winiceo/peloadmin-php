...
use Leven\Http\Requests\API2\StoreVerifyCode;
use Leven\Http\Requests\API2\CreateRegisterVerifyCodeRequest;
...
class VerifyCodeController extends Controller
{
     ...
     * 创建注册验证码.
     *
     * @param \Leven\Http\Requests\API2\CreateRegisterVerifyCodeRequest $request
     * @return mixed
     ...
     * 创建并发送用户验证码.
     *
     * @param \Leven\Http\Requests\API2\StoreVerifyCode $request [description]
     * @return mixed
     * @author Seven Du <shiweidu@outlook.com>
     */
    ...
    protected function send(string $account, string $channel = '', array $data = [])
    {
        ...
        $model->notify(
            new \Leven\Notifications\VerificationCode($model)
        );
    }
